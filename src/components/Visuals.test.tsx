...
describe("cinematic tailoring visuals",()=>{
  ...
  it("exposes visible garment transformation state",()=>{const {rerender}=render(<GarmentMorph garment="dress"/>);expect(screen.getByRole("img")).toHaveAttribute("data-shortened","false");rerender(<GarmentMorph garment="dress" services={["shortening","narrowing"]}/>);expect(screen.getByRole("img")).toHaveAttribute("data-shortened","true");expect(screen.getByRole("img")).toHaveAttribute("data-narrowed","true");expect(document.querySelector(".hem-new")).toBeInTheDocument()});
